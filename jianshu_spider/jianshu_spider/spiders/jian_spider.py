...
class JianSpiderSpider(scrapy.Spider):
    ...
    def parse_followers(self,response):
        ...
        if user_lis:
            page += 1
            yield Request(url='https://www.jianshu.com/u/{0}/followers?page={1}'.format(user_slug,page)
    ...
    def parse(self, response):
        ...
        for url in user_data_lst:
            ...
            yield Request(url='https://www.jianshu.com/u/{}'.format(user_slug)
                          ,headers=self.base_headers
                          ,callback=self.parse_seeduser
                          ,meta={'slug':user_slug})

            yield Request(url='https://www.jianshu.com/u/{}/followers'.format(user_slug)
                          , headers=self.ajax_headers
                          , callback=self.parse_followers
